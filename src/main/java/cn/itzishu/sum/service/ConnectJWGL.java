package cn.itzishu.sum.service;

import cn.itzishu.sum.utils.B64;
import cn.itzishu.sum.utils.RSAEncoder;
...
public class ConnectJWGL {
    ...
    private final String url = "http://jwxt.swun.edu.cn";
    ...
    // 查询学生信息
    public Map getStudentInformaction() throws Exception {
        ...
        response = connection.cookies(cookies).ignoreContentType(true).execute();
        JSONObject jsonObject = JSON.parseObject(response.body());
        map.put("xh_id",jsonObject.getString("xh_id"));
        map.put("xbm",jsonObject.getString("xbm"));
        map.put("mzm",jsonObject.getString("mzm"));
        map.put("jg_id",jsonObject.getString("jg_id"));
        map.put("bh_id",jsonObject.getString("bh_id"));
        map.put("zszyh_id",jsonObject.getString("zszyh_id"));
        map.put("xjztdm",jsonObject.getString("xjztdm"));
        map.put("njdm_id",jsonObject.getString("njdm_id"));
        map.put("zjhm",jsonObject.getString("zjhm"));
        map.put("zzmmm",jsonObject.getString("zzmmm"));
        map.put("xm",jsonObject.getString("xm"));
    ...
    // 获取课表信息
    public List getStudentTimetable(int year , int term) throws Exception {
        ...
        for (Iterator iterator = timeTable.iterator(); iterator.hasNext();) {
            ...
            map.put("xqjmc",lesson.getString("xqjmc"));
            map.put("jc",lesson.getString("jc"));
            map.put("kcmc",lesson.getString("kcmc"));
            map.put("xm",lesson.getString("xm"));
            map.put("xqmc",lesson.getString("xqmc"));
            map.put("cdmc",lesson.getString("cdmc"));
            map.put("zcd",lesson.getString("zcd"));
            list.add(map);
